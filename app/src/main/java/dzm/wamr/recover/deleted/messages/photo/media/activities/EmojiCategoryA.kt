...
import dzm.wamr.recover.deleted.messages.photo.media.databinding.ActivityEmojiCategoryBinding
import dzm.wamr.recover.deleted.messages.photo.media.util.*

class EmojiCategoryA : AppCompatLocaleActivity() {

    lateinit var binding:ActivityEmojiCategoryBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        Utils.setLocale(Prefs.getString(Const.APP_LANGUAGE_CODE,Const.DEFAULT_LANGUAGE_CODE), this, javaClass,false)
        binding=DataBindingUtil.setContentView(this,R.layout.activity_emoji_category)
        initControl()
        actionControl()
    }
    ...
    private fun actionControl() {
        ...
        binding.tabSmiles.setOnClickListener(DebounceClickHandler(View.OnClickListener {
            moveToCategory(0)
        }))
        binding.tabPeople.setOnClickListener(DebounceClickHandler(View.OnClickListener {
            moveToCategory(1)
        }))
        binding.tabNature.setOnClickListener(DebounceClickHandler(View.OnClickListener {
            moveToCategory(2)
        }))
        binding.tabPlaces.setOnClickListener(DebounceClickHandler(View.OnClickListener {
            moveToCategory(3)
        }))
    }
    ...
    private fun moveToCategory(position: Int) {
        if (AdConst.IS_AD_SHOW) {
            InterstitialManager.preLoadAllScreenInterstital?.showInterstitial(this@EmojiCategoryA,
                object: AdCallback {
                    override fun onAdComplete(isCompleted: Boolean) {
                        InterstitialManager.preLoadAllScreenInterstital?.requestInterstitialAd(this@EmojiCategoryA,
    ...
    private fun MoveToNextStepAfterAd(position: Int) {
        val intent = Intent(binding.root.context,EmojiA::class.java)
    ...
    private fun initControl() {
        loadAppNative(this@EmojiCategoryA)
    }
