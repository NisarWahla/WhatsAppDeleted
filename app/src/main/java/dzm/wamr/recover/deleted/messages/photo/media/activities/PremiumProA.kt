...
import dzm.wamr.recover.deleted.messages.photo.media.databinding.ActivityPremiumProBinding
import dzm.wamr.recover.deleted.messages.photo.media.util.*

class PremiumProA : AppCompatLocaleActivity() {

    lateinit var binding: ActivityPremiumProBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding=DataBindingUtil.setContentView(this,R.layout.activity_premium_pro)
    ...
    private fun purchasePackage() {
        if (LoadingDataClass.isNetworkAvailable(this)) {
            ...
            if (chooseString.equals("weeklypro")) {
            ...
            else if (chooseString.equals("weekly")) {
                //Toast.makeText(this, "Clicked 0", Toast.LENGTH_SHORT).show()
                iapConnector.subscribe(
                    this@PremiumProA,
        ...
        iapConnector.addSubscriptionListener(object : SubscriptionServiceListener {
            ...
            override fun onPricesUpdated(iapKeyPrices: Map<String, DataWrappers.ProductDetails>) {
                ...
                if (iapKeyPrices.entries.isNotEmpty()) {
                    try {
                        weeklyProPrice="" + iapKeyPrices[binding.root.context.getString(R.string.weeklypro_id)]?.price.toString()
                        binding.tvPriceWeek.text = weeklyProPrice
                    } catch (e: Exception) {
    ...
    private fun showInterstitialAndMoveMain() {
        ...
        {
            InterstitialManager.preLoadAllScreenInterstital?.showInterstitial(this@PremiumProA,object: AdCallback {
                override fun onAdComplete(isCompleted: Boolean) {
                    InterstitialManager.preLoadAllScreenInterstital?.requestInterstitialAd(this@PremiumProA,Const.AD_NEXT_REQUEST_DELAY)
                    moveToMain()
                }
            },true)
